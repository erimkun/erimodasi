...
const QUALITY_TAG = process.env.PERF_QUALITY === 'standard' ? 'standard' : 'boost';
...
test('captures FPS with full user journey flow', async ({ page }) => {
  await page.goto(`/?quality=${QUALITY_TAG}`, { waitUntil: 'domcontentloaded' });

  const loggerAutoStarted = await page.evaluate(() => (window as any).__FPS_LOGGER_AUTO__ === true);
  if (!loggerAutoStarted) {
  ...
  const hasEnter = await enterButton.isVisible({ timeout: 45_000 }).catch(() => false);
  if (hasEnter) {
  ...
  const hasCanvas = await viewerCanvas.isVisible({ timeout: 12_000 }).catch(() => false);
  ...
  if (hasCanvas) {
    ...
    // Fallback sampling window when WebGL canvas is unavailable in headless env.
    await page.waitForTimeout(6500);
  }

  // Stop logger to finalize a stable payload snapshot.
  await page.keyboard.press('f');
  ...
  const filePath = join(outputDir, `fps-playwright-${QUALITY_TAG}-${now}.json`);
